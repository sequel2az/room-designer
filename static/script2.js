function _stripPhoneFormat(e, val) {
  ...
  if (e != null) {
    ...
    if (matches && matches.length > 0) return val.match(regex).join("")
    else return val
  } else return val
}
    ...
    if (isValidEmail(email)) {
      if (isValidPhone(phone)) {
        if (isValidZip(zip)) {
          ...
          var theurl =
            "https://stage.tailoredliving.com/_Ajax/xpartyconsultform.ashx?firstName=" +
            fname +
            "&" +
            "lastName=" +
            lname +
          ...
          xmlhttp.onreadystatechange = function() {
            if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
              ...
              if (result.Message == "thankyou") {
                ...
                //       alert(result.Message);
                window.location.href = "/nolocation.asp"
              } else {
                alert(result.Message)
              }
            }
          ...
          $("#ih-resultLbl")
            .text("Invalid Zip or Postal Code.")
            .css("color", "red")
